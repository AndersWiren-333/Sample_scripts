# DESCRIPTION: This script looks at a list of differentially expressed features (genes/unannotated transcripts) between some (two in this case) conditions/treatments/tissues. For each feature in the
# list the script evaluates whether the feature is likely to be an (as yet unannotated) exon of the feature immediately before it on the relevant contig. The criteria for the evaluation are closeness
# on the contig (closer than a certain cutoff value, supplied at the command line) and whether the feature is up/down regulated in the same direction as the previous feature (above an offset foldchange
# cutoff supplied at the command line). The script produces an updated genelist with a new column ("parent gene") inserted. This new column holds a numerical value indicating to which "parent gene"
# each feature is likely to belong.
# end description
...
my $usage = "Usage error for script ${0}. Correct usage: 'perl $0 \$genelist \$exon_dist_cut \$OFC_cut \$outname'\n\nwhere".
...
"\t\$OFC_cut is the lowest offset foldchange a feature needs to have to be considered for grouping with another feature\n".
...
for(my $c=0; $c<=$#matrix; $c++)
	...
	# If it is any other line
		...
		{
		# If this feature is on the same contig as the previous one, is close enough and has an offset foldchange above the cutoff value
		if(($arr[$contig_ind] eq $matrix[$previous][$contig_ind]) and ($dist <= $exon_dist_cut) and ($ofc_current >= $OFC_cut) and ($ofc_previous >= $OFC_cut))
			{
			unshift(@arr, $parent_gene);
			push(@new_matrix, [@arr]);
			}
...
my $new_sign_ind = $sign_ind+1;
my $new_ofc_ind = $ofc_ind+1;

# Sort the new updated matrix on significance of differential expression (descending from "YES" to "NO") and offset foldchange (descending)
@new_matrix = sort { $b->[$new_sign_ind] cmp $a->[$new_sign_ind] || $b->[$new_ofc_ind] <=> $a->[$new_ofc_ind] } @new_matrix;

# Add new header line to new matrix
