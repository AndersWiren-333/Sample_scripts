...
# one in each column) **WITHOUT HEADERS** and filters out short and lowly expressed features. The exclusion is based on expression within groups of replicate
# samples/libraries that are defined in a sample name textfile. Only those features/genes/reads will be retained where
#
...
#		sample_4,queen
#
# The column of the matrix containing the lengths of features is given as an argument at the command line (e.g. 9), or, if there isn't a length column but
# the feature IDs themselves contain length information ("NC_123.4_startposition_stopposition"), this argument is set to "id".
#
# Example usage:
# 'perl abundance_and_length_filter_for_matrices.pl gene_expression_matrix_2.csv sample_names.txt filtered_expression_matrix.csv lengthcol 19 1 2 n 20 n'
#
...
my $usage2 = "Correct usage: 'perl $0 \$gene_expression_matrix.csv \$sample_names.txt \$outfile_name \$lengthcol \$length_limit \$w_groups \$x_samples \$x_perc_samples \$y_readcount \$z_expr_mean\n\nwhere".
...
"\t\$lengthcol is the column number containing the lengths of features. If the feature IDs contain the length information, set this argument to 'id'\n".
...
my $lengthcol = shift(@pars) or die $usage;
my $length_cutoff = shift(@pars) or 0;
...
# Read matrix
my @matrix=fileTools::read_table($expression_matrix, "csv");
my @filtered_matrix=();

# Create a proposed, logical name for the outfile (very good for documentation purposes, but may be too long for some file systems/harddrives) in case the user
# wants to rename the outfile after it has been produced.
my $proposed_outname="";
...
# Loop over the expression matrix and filter on length and abundance
for(my $c=0; $c<=$#matrix; $c++)
	...
	# or from the length of the feature ID (if the Feature_ID format is 'ACTGGCTA...'), or take the length from the specified length column
	my $length="Faster_Tinne";		# If this fails to be updated to a number (as it should) we will get a warning (=good)

	# If length information is contained in feature IDs...
	if($lengthcol eq "id")
		{
		# If the ID is a nucleic acid sequence ("ACTGTGUCAT...")
		if($matrix[$c][0] =~ /^[ACTGU]+$/m)	{	$length = length($matrix[$c][0]);	}
		
		# If the ID is of the format "NC_123.4_startpos_stoppos"
		else
			{
			my @feat_name = split("_", $matrix[$c][0]);
			my $last = (scalar(@feat_name))-1;
			my $nextlast = $last-1;
			$length=($feat_name[$last])-($feat_name[$nextlast])+1;
			}
		}
		
	# If lentgh information is in a separate comma
	else	{	$length = $matrix[$c][$lengthcol];	}
	...
	# Loop over groups
	for(my $d=0; $d<=$#group_indices; $d++)
		{
		my @indices = @{$group_indices[$d]};
	...
	if(($pass_abund_criteria>=$w_groups_cutoff) and ($pass_length eq "yes"))	{	push(@filtered_matrix, $matrix[$c]);	}
	} # Loop over lines in infile ends here

# Write filtered matrix
fileTools::write_table(\@filtered_matrix, "csv", $outname, "lin");
	
print("\n\tProposed outname: $proposed_outname\n\n");
