...
my $usage = "perl length_corr.pl gene_expr_matrix.csv";
my $infile = shift or die $usage;

if(!open(IN, $infile))	{	die "Couldn't open $infile";	}
if(!open(OUT, ">>lc_$infile"))	{	die "Couldn't create outfile";	}
...
while(<IN>)
	...
	else	{	die "Couldn't parse sequence id!\n";	}
	my $corr_factor=(1000/$length);
	my @corr_arr=();
	push(@corr_arr, $arr[0]);
	for(my $i=1; $i<scalar(@arr); $i++)
		{
		my $corr_val=($arr[$i])*$corr_factor;
		push(@corr_arr, $corr_val);
		}
	my $outline=join(",", @corr_arr);
	print(OUT $outline, "\n");
	}
